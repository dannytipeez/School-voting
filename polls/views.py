...
def poll_detail(request, poll_id):
    ...
    if request.user:
        department = str(request.user.school)
    else:
        department = "N/A"
    if not poll.active:
    ...
    image = ""
    choices_list, images_list = [], []
    for choice in poll.choice_set.all():
        choices_list.append(choice)
    for choice in choices_list:
        choice_image_url = str(choice.image.url)
        images = (image, choice_image_url)
        new_image = "".join(images)
        images_list.append(new_image)

    #create a dictionary
    choice_image_zip = zip(choices_list, images_list)
    choice_image_dict = dict(choice_image_zip)
    print(choice_image_dict)

    for choice, image in choice_image_dict.items():
        print(choice, image)
        
    context = {
        'poll': poll,
        'loop_time': range(0, loop_count),
        'choice_image_dict' : choice_image_dict,
